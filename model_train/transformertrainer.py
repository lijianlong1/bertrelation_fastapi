...
max_model_version = model_version()['model_version_id']
this_model_version = int(max_model_version) + 1


#  在此需要安装对应的软件包，用于后面的代码编写#
# 相关的软件包有如下相关的软件包全部已经下载完成，需要进行莫i性的训练
# from BERT_Pytorch_Fintuing.loader import load_dev,load_train,map_id_rel
...
def transtraindata(num_epochs=50, learning_rate=0.001, batch_size=1):
    setup_seed(44)

    # rel2id, id2rel = map_id_rel()
    rel2id, id2rel, data_train = map_id_rel_from_sql()
    ...
    data = data_train  # 将所有的训练数据都送给eval数据
    dev_text = data['text']
    dev_mask = data['mask']
    ...
    def train(net, dataset, num_epochs, learning_rate, batch_size):
        ...
        for epoch in range(num_epochs):
            ...
            if acc > pre:
                pre = acc
                torch.save(model, './best_model_save/GRU_'+str(this_model_version) + '.pth')  # 此处的模型的名称需要进行相应的替换
        return 0
    ...
    train(model, train_dataset, num_epochs, learning_rate, batch_size)  # batch_size可以设置的稍微大一些，以便后续的调用，此时的batch-size大小应该为4
    # eval(model,dev_dataset,8)
    return "trained"
